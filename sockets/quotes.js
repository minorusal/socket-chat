...
const cambiaEstatus = async (cliente, io) => {
  cliente.on('cambia-estatus', async (payload) => {
    console.log(`[Cambiar Estatus] Cotización ${payload.cotizacion} cambiará a estatus ${payload.estatus}`);
    console.log(payload);
    ...
    try {
      ...
      console.log(`[Cambiar estatus] Se cambio el estatus de la cotizacion ${payload.cotizacion}`);
      ...
      console.log(`[Cambiar estatus] Ocurrió un error al cambiar el estatus de la cotización ${payload.cotizacion}`);
      io.to(`'${payload.cotizacion}'`).emit('cambio-estatus', {
        error: true,
...
// Crear cotización
const creaCotizacion = async (cliente, io) => {
  // Se debe de recibir lo que recibe por body la REST API
  cliente.on('crea-cotizacion', async (payload) => {
    try {
      console.log(`[Crea Cotización] Con el siguiente payload:`);
      console.log(payload);
      ...
      console.log(`[Crea Cotización] Cotizaciones creadas:`);
        ...
        for (let i = 0; i < cotizacionesCreadas.length; i++) {
          ...
          if (usuarios) {
            for (let j = 0; j < usuarios.length; j++) {
              ...
              console.log(`[Crea Cotización] Enviando evento a usuario ${userID}.`);
      ...
      console.log(`[Crea Cotización] No se pudo crear la cotización.`);
      // FIXME: Revisar con iOS si esto funciona
...
const creaCotizacionHija = async (cliente, io) => {
  cliente.on('crea-hija', async (payload) => {
    console.log(`[Crea Cotización Hija] Se creará una cotización hija para la cotización padre ${payload.cot_padre_id}`);
    console.log(payload);
    ...
    try {
      ...
      console.log(`[Crea Cotización Hija] Se creó una cotización hija.`);
      ...
      console.log(`[Crea Cotización Hija] Obtiene detalles de la cotización hija recién creada:`);
      ...
      console.log(`[Crea Cotización Hija] Se creó una cotización hija. Emitiendo a ${payload.cot_padre_id}`);
      ...
      console.log(`[Crea Cotización Hija] No fue posible crear una cotización hija.`);
      io.to(`'${payload.cot_padre_id}'`).emit('cotizacion-actualizada', {
        error: true,
...
module.exports = { cambiaEstatus, creaCotizacion, creaCotizacionHija };
